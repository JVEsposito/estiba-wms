...
async function saveJson(form, errorNode, basePath, label) {
    errorNode.textContent = ''; if (!state.season) { errorNode.textContent = 'No existe una temporada transversal. Debes crearla en Accesos.'; return; }
    const data = Object.fromEntries(new FormData(form)); const id = data.id; delete data.id;
    data.temporada_id = state.season.id;
...
elements.importForm.addEventListener('submit', async (event) => { event.preventDefault(); elements.importError.textContent = ''; if (!state.season) { elements.importError.textContent = 'No existe una temporada transversal. Debes crearla en Accesos.'; return; } const data = new FormData(elements.importForm); data.set('temporada_id', state.season.id); setBusy(true, 'Leyendo y validando planilla…'); try { const response = await api('/api/administracion/validacion/importaciones/previsualizar', { method: 'POST', body: data }); renderImportPreview(response.data); await loadAdministration(state.season.id); } catch (error) { elements.importError.textContent = error.message; } finally { setBusy(false); } });
