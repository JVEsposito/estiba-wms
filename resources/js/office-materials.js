...
function renderAll() { renderItems(); renderDestinations(); renderDispatches(); renderInventory(); renderMetrics(); }
...
async function loadAll() {
    ...
    const [catalog, dispatches, inventory] = await Promise.all([
        catalogAdmin ? Promise.all([api('/api/administracion/materiales/items'), api('/api/administracion/materiales/destinos')]) : api('/api/materiales/catalogo'),
        api('/api/materiales/despachos'), api('/api/materiales/inventario'),
    ]);
    if (catalogAdmin) { state.items = catalog[0].data; state.destinations = catalog[1].data; } else { state.items = catalog.items; state.destinations = catalog.destinos; }
